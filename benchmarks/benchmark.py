...
class BenchmarkRunner:
    ...
    def run_benchmark(self, project_path: str, name: str) -> Dict[str, Any]:
        ...
        result = {
            ...
            "files_per_second": round(file_count / execution_time, 2) if execution_time > 0 else 0,
            "lines_per_second": round(line_count / execution_time, 2) if execution_time > 0 else 0,
            "timestamp": time.strftime("%Y-%m-%d %H:%M:%S")
        }
        
        print(f"✅ Completed in {execution_time:.2f}s")
        print(f"   Files: {file_count}, Lines: {line_count}")
        print(f"   Speed: {result['files_per_second']:.2f} files/s, {result['lines_per_second']:.2f} lines/s")
    ...
    def print_summary(self) -> None:
        ...
        print(f"Overall throughput: {total_files/total_time:.2f} files/s, {total_lines/total_time:.2f} lines/s")
...
def main():
    ...
    parser.add_argument(
        "--projects",
        nargs="+",
        help="Project paths to benchmark (format: name:path)"
    )
    parser.add_argument(
        "--output",
