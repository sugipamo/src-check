    ...
    def get_adjusted_thresholds(self, file_path: Path) -> Dict[str, Dict[str, float]]:
        ...
        for metric in ['complexity', 'coupling', 'line_length', 'function_length']:
            ...
            if metric == 'complexity':
                thresholds[metric]['cyclomatic'] = self.rule_manager.get_adjusted_threshold(
                    metric, 'cyclomatic', context
                )
                thresholds[metric]['cognitive'] = self.rule_manager.get_adjusted_threshold(
                    metric, 'cognitive', context
                )
            elif metric == 'coupling':
                thresholds[metric]['max_dependencies'] = self.rule_manager.get_adjusted_threshold(
                    metric, 'max_dependencies', context
                )
                thresholds[metric]['max_imports'] = self.rule_manager.get_adjusted_threshold(
                    metric, 'max_imports', context
                )
            elif metric == 'line_length':
                thresholds[metric]['max_length'] = self.rule_manager.get_adjusted_threshold(
                    metric, 'max_length', context
                )
            elif metric == 'function_length':
                thresholds[metric]['max_lines'] = self.rule_manager.get_adjusted_threshold(
