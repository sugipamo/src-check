...
class MarkdownFormatter(BaseFormatter):
    ...
    def format(self, results: Dict[str, List[CheckResult]], kpi: KpiScore) -> str:
            ...
            for file_path, file_results in sorted(results.items()):
                if file_results:
                    ...
                    sorted_results = sorted(
                        file_results,
                        key=lambda r: [Severity.CRITICAL, Severity.HIGH,
                                     Severity.MEDIUM, Severity.LOW,
                                     Severity.INFO].index(r.severity)
                    )
                    
                    for result in sorted_results:
                        severity_badge = self._get_severity_badge(result.severity)
                        location = f"L{result.line}" if result.line else "File"
        ...
        # Footer
        output.append("---")
        output.append("*Report generated by [src-check](https://github.com/yourusername/src-check)*")
    ...
    def _format_recommendations(self, kpi: KpiScore, results: Dict[str, List[CheckResult]]) -> str:
        """Generate recommendations based on results."""
        recommendations = []
        
        if kpi.critical_issues > 0:
            recommendations.append("🚨 **Priority 1**: Fix all critical security issues immediately")
            
        if kpi.high_issues > 0:
            recommendations.append("⚠️ **Priority 2**: Address high-severity issues before deployment")
            
        if kpi.overall_score < 70:
            recommendations.append("📈 **Quality**: Consider refactoring to improve code quality")
            
        if "test_quality" in kpi.category_scores and kpi.category_scores["test_quality"] < 60:
            recommendations.append("🧪 **Testing**: Increase test coverage and quality")
            
        if not recommendations:
            recommendations.append("✅ **Great job!** Keep maintaining high code quality standards")
