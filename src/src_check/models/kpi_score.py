...
class KPIScore:
    ...
    def calculate_from_results(
        ...
        for result in check_results:
            ...
            category_scores[category].raw_score += severity_impact * result.failure_count
            ...
            category_scores[category].issues_by_severity[severity_key] += result.failure_count
        
        # Ensure scores don't go below 0
        for category in category_scores.values():
            category.raw_score = max(0, category.raw_score)
            category.weighted_score = category.raw_score * category.weight
