...
class DocstringVisitor(ast.NodeVisitor):
    ...
    def visit_ClassDef(self, node: ast.ClassDef) -> None:
        ...
        if not docstring:
            self.issues.append((node.lineno, f"Missing docstring for class '{node.name}'"))
        
        self.generic_visit(node)
        
    def _check_function_doc(self, node: ast.FunctionDef) -> None:
        ...
        if not docstring:
            self.issues.append((node.lineno, f"Missing docstring for function '{node.name}'"))
            return
            
        # Check docstring completeness
        self._check_docstring_completeness(node, docstring)
        
    def _check_docstring_completeness(self, node: ast.FunctionDef, docstring: str) -> None:
        ...
        if has_return and node.name != '__init__':
            ...
            if node.returns:
                ...
                if (hasattr(node.returns, 'id') and node.returns.id == 'None') or \
                   (isinstance(node.returns, ast.Constant) and node.returns.value is None) or \
                   (isinstance(node.returns, ast.NameConstant) and node.returns.value is None):
                    return
