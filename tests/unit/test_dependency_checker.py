...
class TestDependencyChecker:
    ...
    def test_unused_dependency_detection(self):
        ...
        assert any("unused-package" in failure.message for r in results for failure in r.failure_locations)
    ...
    def test_unpinned_version_detection(self):
        ...
        assert any("requests" in failure.message for r in results for failure in r.failure_locations)
        assert any("numpy" in failure.message for r in results for failure in r.failure_locations)
    ...
    def test_dev_prod_mixing_detection(self):
        ...
        assert any("pytest" in failure.message for r in results for failure in r.failure_locations)
        assert any("black" in failure.message for r in results for failure in r.failure_locations)
...
"""
    ...
    def test_check_project_method(self):
        ...
        assert any(r.rule_id == "DEP002" and any("unused" in f.message for f in r.failure_locations) for r in results)
...
"""
        ...
        assert not any("pillow" in f.message for r in results for f in r.failure_locations)
        assert not any("opencv-python" in f.message for r in results for f in r.failure_locations)
