...
from src_check.models import Severity
...
'''
        ...
        if result:
            assert not any("PERF001" in issue.message for issue in result.failure_locations)
...
'''
        ...
        if result:
            assert not any("PERF004" in issue.message for issue in result.failure_locations)
...
'''
        ...
        if result:
            assert not any("PERF003" in issue.message for issue in result.failure_locations)
